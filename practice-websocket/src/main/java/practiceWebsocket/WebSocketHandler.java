...
import practiceWebsocket.service.MessageManager;
import practiceWebsocket.service.RoomManager;
...
public class WebSocketHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage textMessage)
        ...
        SocketProtocol webSocketSocketProtocol = mapper.readValue(textMessage.getPayload(),
        ...
        switch (action) {
            case JOIN:
                ...
                messageManager.roomBroadCastSystemMessage(session.getId() + "님이 접속하였습니다.",
                    sessions);
                break;
            case MESSAGE:
                sessions = roomManager.getAllSessionInRoom(roomId);
                sessions.remove(session);
                messageManager.roomBroadCastUserMessage(session, SocketAction.MESSAGE, payload,
                    sessions);
                break;
        }
    }
